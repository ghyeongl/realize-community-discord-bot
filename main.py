...
async def on_message(message):
    ...
    # 디버그 채널인 경우 모두 무시
    if channel.id == database.get_id_channel(8, 3):
        return

    # 로그 정보 생성
    user_id = database.get_disc_author(author)
    log_content = message.content
    if isinstance(channel, discord.channel.DMChannel):
        log_author = user_id
        log_channel = channel.recipient.name + "#" + channel.recipient.discriminator
        log_etc = {"author_id": author.id}
    else:
        log_author = user_id
        log_channel = channel.name
        log_etc = {"channel_id": channel.id, "Nick": author.nick}
    ...
    try:
        ...
        log.error("discord.errors.HTTPException", details=str(err))
        await debug_ch.send(f'**[discord.errors.HTTPException]** {err}')
        await debug_ch.send(file=discord.File(log.f))

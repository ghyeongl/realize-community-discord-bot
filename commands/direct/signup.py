...
async def fork(channel, message, author, client):
    ...
    # 첫 번째 입력
        ...
        await person_agree(channel, message, author)
...
# 새 사용자로 등록된 유저
async def person_agree(channel, message, author):
...
async def reg_confirm(channel, author, client, reg_info):
    ...
    if security_status == 0:
        if reg_info['Number'] >= 15:
            ...
            await reg_confirmed(author, client)
...
# 승인시 권한 해제 절차
async def reg_confirmed(author, client):
    log.call(__name__, reg_confirmed.__name__, author=author.id)

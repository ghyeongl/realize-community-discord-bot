...
from commands.devops.management import security_status, keys, names
...
async def fork(channel, message, author, client):
    ...
    # 첫 번째 입력
        ...
        await person_agree(channel, message, author.id)

    # 두 번째 입력
    elif member_info[author.id]["Name"] == "":
        await person_name(channel, message, author.id)

    # 세 번째 입력
    elif member_info[author.id]["Number"] == "":
        await person_number(channel, message, author.id)

    # 네 번째 입력
    elif member_info[author.id]["Key"] == "":
        await person_key(channel, message, author.id)

    # 작성 완료 목록에 포함되지 않은 경우
    elif author.id not in confirm_pending:
        await person_confirm(channel, message, author, client)

    # 그 외 (새 멤버 캐시와, 작성 완료 목록에 모두 있는 사용자)
    else:
        await confirmation(channel, message, author.id)
...
async def meet(channel, author_id):
    ...
    member_info[author_id] = {"Confirm": "", "Name": "", "Number": "", "Key": ""}
...
async def person_agree(channel, message, author_id):
    log.call(__name__, person_agree.__name__, author_id=author_id, channel_id=channel.id)
    if message.content.startswith('예'):
        member_info[author_id]["Confirm"] = True
...
async def person_name(channel, message, author_id):
    log.call(__name__, person_name.__name__, author_id=author_id, channel_id=channel.id)
    member_info[author_id]["Name"] = message.content
...
async def person_number(channel, message, author_id):
    log.call(__name__, person_number.__name__, author_id=author_id, channel_id=channel.id)
    if message.content.isdigit():
        member_info[author_id]["Number"] = int(message.content)
        await channel.send(f"{member_info[author_id]['Number']}기로 입력되었어요. (아닐 시 '초기화' 입력)")
        await channel.send('제공받은 인증키를 입력해주세요! (없다면 "없음" 입력)')

    elif message.content.split('기')[0].isdigit():
        member_info[author_id]["Number"] = int(message.content.split('기')[0])
        await channel.send(f"{member_info[author_id]['Number']}기로 입력되었어요. (아닐 시 '초기화' 입력)")
        await channel.send('제공받은 인증키를 입력해주세요! (없다면 "없음" 입력)')
...
# 리얼라이즈 회원인지
async def person_key(channel, message, author_id):
    log.call(__name__, person_key.__name__, author_id=author_id, channel_id=channel.id)
    if message.content.startswith('없음'):
        member_info[author_id]["Key"] = False
    else:
        member_info[author_id]["Key"] = message.content

    value = f"동의여부: {member_info[author_id]['Confirm']}, 이름: {member_info[author_id]['Name']}, " \
            f"기수: {member_info[author_id]['Number']}, 인증키: {member_info[author_id]['Key']}"
    await channel.send(f'입력된 값이예요: {value}. 등록 요청을 전송할까요? (예/아니오)')


# 전송여부 판단
async def person_confirm(channel, message, author, client):
    ...
    if message.content.startswith('예'):
        if member_info[author.id]['Key']:
            ...
            await new_request(message, author.id, client)
...
async def reg_confirm(channel, author, client, reg_info):
    ...
    if security_status == 0:
        if reg_info['Number'] >= 15:
            ...
            await reg_confirmed(channel, author, client, reg_info)
...
# 승인시 권한 해제 절차
async def reg_confirmed(channel, author, client, reg_info):
    log.call(__name__, reg_confirmed.__name__, author_id=author.id, channel_id=channel.id)
    database.add_member(author.id, author.name, author.discriminator, reg_info)
    guild = client.get_guild(database.get_id_server())
    member = guild.get_member(author.id)

    try:
        if member is not None:
            await member.add_roles(client.get_guild(database.get_id_server()).get_role(database.get_id_roles("Makers")),
                                   reason="캡챠가 부여함")
            await member.edit(nick=reg_info["Name"])
            await channel.send('등록이 완료되었어요. 커뮤니티로 돌아가 확인해보세요!')
            del member_info[author.id]
        else:
            log.error("member is None", __name__, reg_confirmed.__name__, member=member)
            database.del_member(author.id)
            await channel.send('앗! 서버에 존재하지 않는 유저예요. 이 증상이 지속되면, 관리자에게 연락하세요.')
            await reset(channel, author.id)

    except discord.errors.Forbidden as e:
        log.error("discord.errors.Forbidden", __name__, reg_confirmed.__name__, content=e)
        await channel.send('알 수 없는 에러가 발생했어요. 그러면 당신은 혹시 신적인 존재..?')
        await reset(channel, author.id)
...
async def confirmation(channel, message, author_id):
    log.call(__name__, confirmation.__name__, author_id=author_id, channel_id=channel.id)
